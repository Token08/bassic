...
| START | `startTime` | テキスト | no | 公開する場合は必ず入力。`19:00` のように半角数字と `:` で入力 |
...
`日付` と `START` は、公開するイベントでは管理画面側で入力確認します。
公開するイベントは、イベント名を4文字以上で入力します。短すぎる仮タイトルのまま公開しないための確認です。
...
公開する料金は `1,200円` のような金額、または `時価`、`お問い合わせ`、`応相談` のように来店前に意味が分かる表記にします。
...
公開するプランは、プラン名を4文字以上、説明を12文字以上で入力します。料金、人数、利用内容が分かる短い一文にすると、カード表示でも読みやすくなります。
公開する料金は `4,000円〜 / 1名`、`応相談`、`お問い合わせ` のように意味が分かる表記にします。
...
見出しは6文字以上、説明は20文字以上で、利用内容や注意点が分かる文章にします。
PDFを差し替えた時は `pdfUrl` も新しいURLへ変更し、管理画面の「PDFを開いて確認」で正しいPDFが開くか確認します。
...
説明は1〜2文で、投稿内容や見てほしいポイントが分かるようにします。
...
公開する自由セクションは、見出しを6文字以上、本文を20文字以上で入力します。
リンクボタンを出す場合は、リンク文言とリンクURLを両方入力します。片方だけでは公開前チェックで止まります。
短すぎる文章は公開前チェックで止まるため、誰に何を伝えるセクションか分かる文章にしてください。
